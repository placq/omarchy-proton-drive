...
raise SystemExit(0 if status.get("version") == "1.0.2" and status.get("apiVersion") == 1 else 1)
' <<<"$status_json"; then
  printf 'OK   daemon/API version       1.0.2 / 1\n'
else
  printf 'WARN daemon/API version mismatch (expected 1.0.2 / 1)\n'; warn=$((warn+1))
fi
...
if [[ -f $plugin_manifest ]]; then
  if python3 -c 'import json,sys; raise SystemExit(0 if json.load(open(sys.argv[1])).get("version") == "1.0.2" else 1)' "$plugin_manifest"; then
    printf 'OK   Quattro plugin version   1.0.2\n'
  else
