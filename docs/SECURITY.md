...
Known alpha gaps: the Unix RPC socket relies on its private runtime-directory ownership and mode `0600`; FUSE and QML have not yet completed a hostile-input audit; symlink creation is intentionally unsupported. Real-account writes are enabled only as an alpha feature and remain subject to the dedicated-directory safety rules.
