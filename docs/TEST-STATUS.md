...
| Rename/move/mkdir/trash | native provider operations, non-empty `rmdir` refusal, cache cleanup, descendant-cycle rejection and recursive unsynchronised-data guards | Automated |
| Pin/free space/cache | recursive folder pin, pin refresh, unpinned remote invalidation, revision-bound cache, startup orphan pruning, unpin/evict, unsafe eviction, LRU | Automated |
...
| RPC | version/status/core dispatch, deadline cancellation, real daemon socket, browse/materialize, staged write/commit, restart recovery, Watch transfer events, prompt SIGTERM with an active Watch client | Automated |
