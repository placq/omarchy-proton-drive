...
Verified 2026-08-16 for `0.3.0-alpha.2`. “Automated” means the 61-test provider/core suite and 7-test pure Nautilus helper suite pass in this repository. Desktop and real-account coverage were exercised on Omarchy Quattro workstations.
...
| State/restart safety | SQLite transactions, private WAL, legacy migration, corrupt-state refusal, interrupted upload and persistent staging recovery | Automated |
| Edit/conflict | revision-checked save, remote change, preserved conflict copy, crash-safe finalisation, keep-local, keep-remote and save-both | Automated |
...
| FUSE runtime | real daemon + pyfuse3 mount, browse/open/write/rename/move/delete and service restart recovery | Passed with fake provider and with the installed `0.3.0-alpha.2` package against the dedicated real-account directory |
