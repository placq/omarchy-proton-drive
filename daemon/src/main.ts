...
if (!["fake", "proton-cli"].includes(providerName)) {
  ...
  const stateName = providerName === "fake" ? "state.json" : `state-${providerName}.json`;
  const engine = new DriveEngine(provider, new StateStore(join(dataHome, "omarchy-drive", stateName)), new LocalStorage(join(cacheHome, "omarchy-drive"), join(dataHome, "omarchy-drive")));
